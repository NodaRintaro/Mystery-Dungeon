...
namespace Layer.Infrastructure
{
    ...
    public class QuestData : ScriptableObject, IQuestData
    {
        [SerializeField, Tooltip("クエストのID")] private int _questId;

        [SerializeField, Tooltip("クエストの名前")] private string _questName;

        [SerializeField, Tooltip("クエストの説明")] private string _questDescription;

        [SerializeField, Tooltip("クエストの発行されたダンジョンの階数")] private int _dungeonFloorNum;

        [SerializeField, Tooltip("クエストのクリア条件")] private IQuestClearCondition _clearCondition;

        [Header("ダンジョンのタイルのデータのパス")]
        [SerializeField, Tooltip("タイルのデータのパス")] 
        private string _tileDataPath;

        [SerializeField, Tooltip("ダンジョンに生息するキャラクター達のID")] 
        private int[] _questDungeonCharactersID;

        [SerializeField, Tooltip("クエストが発行されたダンジョンの各フロアの詳細情報")]
