...
namespace Roguelike.Dungeon.System
{
    #region ステージ全体の生成クラス
    /// <summary> ランダムな地形のステージを自動生成するためのロジッククラス </summary>
    public class StageBuilder
    {
        private StageData _stageData = null;
        private RoomData[] _canBuildData = null;
        ...
        public async UniTask<StageData> StageDungeon(int sectionSize, int horizontalSectionNum, int VerticalSectionNum, int minRoomNum, int maxRoomNum, int addRoadNum, string roomDataPath)
        {
            ...
            //ダンジョンのデータを初期化
            _stageData.InitStageData(horizontalSectionNum, VerticalSectionNum);
            ...
            for (int x = 0; x < horizontalSectionNum; x++)
            {
                ...
                {
                    _stageData.SetSectionData(_sectionBuilder.GetNewSection(sectionSize), x, y);
                }
            }
        ...
        private void SelectBuildRoomSection(int minRoomNum, int maxRoomNum)
        {
            ...
            foreach (var item in _stageData.StageSectionsData)
            {
        ...
        {
            foreach (var data in _stageData.StageSectionsData)
            {
                if (data.IsBuildRoom)
                {
        ...
        private void BuildRoads(int addRoadNum)
        {
            ...
            _stageData.StageSectionsData[connectSectionIndex.x, connectSectionIndex.y].IsConnect = true;
        ...
        {
            int widthLength = _stageData.StageSectionsData.GetLength(0);
            int heightLength = _stageData.StageSectionsData.GetLength(1);
            ...
            for (int x = 0; x < widthLength; x++)
            {
                for (int y = 0; y < heightLength; y++)
                {
                    ...
                    if (!_stageData.StageSectionsData[x, y].IsConnect)
                    {
        ...
        private void ConnectAllRoads()
        {
            ...
            for (int x = 0; x < widthLength; x++)
            {
                for (int y = 0; y < heightLength; y++)
                {
                    ...
                    if (!_stageData.StageSectionsData[x, y].IsBuildRoom && GetConnectRoadNum(sectionIndex) <= minRoadNum)
                    {
        ...
        private List<Vector2Int> GetCanConnectSections(Vector2Int connectSectionIndex, bool isConnectOther)
        {
            ...
            SectionData sectionData = _stageData.StageSectionsData[connectSectionIndex.x, connectSectionIndex.y];
        ...
        {
            int roadNum = 0;
            SectionData sectionData = _stageData.StageSectionsData[checkSectionIndex.x, checkSectionIndex.y];
        ...
        private bool TryGetValidSection(out Vector2Int validSection, int x, int y, bool isConnectOtherSection)
        {
            ...
            if (x >= 0 && x < widthLength && y >= 0 && y < heightLength && _stageData.StageSectionsData[x, y].IsConnect == isConnectOtherSection)
            {
        ...
        {
            SectionData baseSection = _stageData.StageSectionsData[baseConnectSectionPos.x, baseConnectSectionPos.y];
            SectionData connectSection = _stageData.StageSectionsData[connectSectionPos.x, connectSectionPos.y];
    ...
    public class SectionBuilder
    {
        ...
        public void BuildRoom(SectionData sectionData, RoomData roomData)
        {
